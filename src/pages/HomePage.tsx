...
import { ArrowLeftIcon, UserIcon, WifiIcon } from '@heroicons/react/24/outline';
...
const FloatingCard = ({
  ...
  <motion.div
    ...
    className={`absolute bg-white/70 backdrop-blur-lg p-4 rounded-2xl shadow-card border border-white/20 animate-float ${className || ""}`}
  >
    ...
    <div className="flex items-center mt-3">
      ...
      <div>
        <p className="text-xs font-bold text-brand-text-primary">{author}</p>
        {subtext && <p className="text-xs text-brand-text-secondary">{subtext}</p>}
      </div>
    </div>
  </motion.div>
);

// --- TopNavbar Subcomponent ---
const TopNavbar = () => {
      ...
      <div className="max-w-screen-xl mx-auto px-6 flex justify-between items-center">
        <div className="flex items-center space-x-3">
          <div className="w-10 h-10 bg-brand-primary-light rounded-full flex items-center justify-center">
            <WifiIcon className="w-6 h-6 text-brand-primary" />
          </div>
        ...
        <nav className="hidden lg:flex items-center space-x-10 text-brand-text-secondary font-semibold">
          ...
          <a href="#pricing" onClick={(e) => handleScroll(e, 'pricing')} className="hover:text-brand-primary transition-colors">Pricing</a>
          <a href="#about" onClick={(e) => handleScroll(e, 'about')} className="hover:text-brand-primary transition-colors">About Us</a>
          <a href="#contact" onClick={(e) => handleScroll(e, 'contact')} className="hover:text-brand-primary transition-colors">Contact</a>
        </nav>
      ...
      <main className="relative pt-48 pb-24 overflow-hidden">
        <div className="max-w-screen-xl mx-auto px-6 grid lg:grid-cols-2 gap-12 items-center">
          <motion.div variants={containerVariants} className="z-10">
            <motion.div variants={itemVariants} className="flex items-center space-x-2 text-sm text-brand-text-secondary font-semibold mb-4">
              <ArrowLeftIcon className="w-4 h-4" />
              <span>Career Advisor Tslun E</span>
            </motion.div>
            <motion.h1 variants={itemVariants} className="text-6xl lg:text-7xl font-black text-brand-text-primary tracking-tighter leading-tight">
              Unlock your career potential with AI
            </motion.h1>
            <motion.p variants={itemVariants} className="mt-6 text-lg text-brand-text-secondary max-w-lg">
              AI Career Advisor & Roadmap Builder App to help you create personalized oaths and interactive roadmaps with AI-powered suggestions.
            </motion.p>
            <motion.div variants={itemVariants} className="mt-10">
              <Link
                to={user ? "/dashboard" : "/signup"}
                className="inline-block px-10 py-4 text-lg font-bold rounded-2xl text-white bg-brand-primary shadow-button-primary hover:-translate-y-1 transition-transform duration-300"
              >
                Get Started
              </Link>
            </motion.div>
